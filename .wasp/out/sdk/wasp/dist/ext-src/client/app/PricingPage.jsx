...
const PricingPage = () => {
    ...
    async function handleBuyNowClick(tierId) {
        var _a;
        if (!user) {
            history.push('/login');
            return;
        ...
        try {
            ...
            if (stripeResults === null || stripeResults === void 0 ? void 0 : stripeResults.sessionUrl) {
                window.open(stripeResults.sessionUrl, '_self');
            }
        }
        catch (error) {
            console.error((_a = error === null || error === void 0 ? void 0 : error.message) !== null && _a !== void 0 ? _a : 'Something went wrong.');
        }
        finally {
    ...
    return (<div className='py-10 lg:mt-10'>
      <div className='mx-auto max-w-7xl px-6 lg:px-8'>
        ...
        <div className='isolate mx-auto mt-16 grid max-w-md grid-cols-1 gap-y-8 lg:gap-x-8 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3'>
          {tiers.map((tier) => (<div key={tier.id} className={cn('relative flex flex-col grow justify-between rounded-3xl ring-gray-900/10 dark:ring-gray-100/10 overflow-hidden p-8 xl:p-10', {
                'ring-2': tier.bestDeal,
                'ring-1 lg:mt-8': !tier.bestDeal,
            })}>
              {tier.bestDeal && (<div className='absolute top-0 right-0 -z-10 w-full h-full transform-gpu blur-3xl' aria-hidden='true'>
              ...
              <div className='mb-8'>
                ...
                <ul role='list' className='mt-8 space-y-3 text-sm leading-6 text-gray-600 dark:text-white'>
                  {tier.features.map((feature) => (<li key={feature} className='flex gap-x-3'>
                      <AiFillCheckCircle className='h-6 w-5 flex-none text-yellow-500' aria-hidden='true'/>
                      {feature}
                    </li>))}
                </ul>
              </div>
              {!!user && !!user.subscriptionStatus ? (<button onClick={handleCustomerPortalClick} aria-describedby='manage-subscription' className={cn('mt-8 block rounded-md py-2 px-3 text-center text-sm font-semibold leading-6 focus-visible:outline focus-visible:outline-2 focus-visible:outline-yellow-400', {
                  ...
                  Manage Subscription
                </button>) : (<button onClick={() => handleBuyNowClick(tier.id)} aria-describedby={tier.id} className={cn({
                    'bg-yellow-500 text-white hover:text-white shadow-sm hover:bg-yellow-400': tier.bestDeal,
                    'text-gray-600  ring-1 ring-inset ring-purple-200 hover:ring-purple-400': !tier.bestDeal,
                }, {
