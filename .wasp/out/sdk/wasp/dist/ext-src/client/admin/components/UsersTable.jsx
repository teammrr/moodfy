...
const UsersTable = () => {
    ...
    const { data, isLoading, error } = useQuery(getPaginatedUsers, {
        ...
        subscriptionStatus: (statusOptions === null || statusOptions === void 0 ? void 0 : statusOptions.length) > 0 ? statusOptions : undefined,
    });
    useEffect(() => {
        setPage(1);
    }, [email, statusOptions]);
      ...
      <div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
        <div className='flex-col flex items-start justify-between p-6 gap-3 w-full bg-gray-100/40 dark:bg-gray-700/50'>
          ...
          <div className='flex items-center justify-between gap-3 w-full px-2'>
            ...
            setEmail(e.currentTarget.value);
              ...
              <div className='flex-grow relative z-20 rounded border border-stroke pr-8 outline-none bg-white transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input'>
                <div className='flex items-center'>
                  {!!statusOptions && statusOptions.length > 0 ? (statusOptions.map((opt, idx) => (<span key={opt} className='z-30 flex items-center my-1 mx-2 py-1 px-2 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary'>
                        {opt ? opt : 'has not subscribed'}
                        <span onClick={(e) => {
                e.stopPropagation();
                setStatusOptions((prevValue) => {
                    return prevValue === null || prevValue === void 0 ? void 0 : prevValue.filter((val) => val !== opt);
                });
            ...
            setStatusOptions((prevValue) => {
                if (prevValue === null || prevValue === void 0 ? void 0 : prevValue.includes(targetValue)) {
                    return prevValue === null || prevValue === void 0 ? void 0 : prevValue.filter((val) => val !== targetValue);
                }
                else if (!!prevValue) {
                    return [...prevValue, targetValue];
                }
                else {
                    return prevValue;
                }
            ...
            {!isLoading && (<div className='max-w-60'>
                ...
                <input type='number' value={page} min={1} max={data === null || data === void 0 ? void 0 : data.totalPages} onChange={(e) => {
                ...
                <span className='text-md text-black dark:text-white'> / {data === null || data === void 0 ? void 0 : data.totalPages} </span>
              </div>)}
          </div>
        </div>
        ...
        {!!(data === null || data === void 0 ? void 0 : data.users) &&
            ((_a = data === null || data === void 0 ? void 0 : data.users) === null || _a === void 0 ? void 0 : _a.length) > 0 &&
            data.users.map((user) => (<div key={user.id} className='grid grid-cols-12 gap-4 border-t border-stroke py-4.5 px-4 dark:border-strokedark  md:px-6 '>
              <div className='col-span-3 flex items-center'>
